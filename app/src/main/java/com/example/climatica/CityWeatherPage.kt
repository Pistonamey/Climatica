...
import com.example.climatica.models.*
import com.example.climatica.network.AirPollutionService
import com.example.climatica.network.GeocodingService
import com.example.climatica.network.WeatherService
import com.example.climatica.R
...
class CityWeatherPage : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_city_weather_page)
    ...
    val mLocationCallback = object : LocationCallback() {
        override fun onLocationResult(p0: LocationResult) {
            ...
            val go:Button=findViewById(R.id.go_btn)

            go.setOnClickListener {
                val city_name:EditText= findViewById(R.id.city_name_search)
                val name:String=city_name.text.toString()
                getLocationDetails(name) }

        }
    }

    private fun getLocationDetails(city_name:String) {
        if (Constants.isNetworkAvailable(this)) {
            ...
            val service: GeocodingService =
                retrofit.create<GeocodingService>(GeocodingService::class.java)

            //make a listcall based on the service
            val listCall: Call<GeocodingResponse> =
                service.getGeocoding(city_name, 5, Constants.APP_ID,)

            showCustomProgressDialog()

            //Performing in the background
            listCall.enqueue(object : Callback<GeocodingResponse> {
                override fun onResponse(
                    ...
                    if (response!!.isSuccessful) {
                        ...
                        val geocodingList: GeocodingResponse = response.body()!!



                        val geocodingResponseJsonString = Gson().toJson(geocodingList)
                        // Save the converted string to shared preferences
                        val editor = msharedPreferences.edit()
                        editor.putString(Constants.GEO_CODING_RESPONSE_DATA, geocodingResponseJsonString)
                        editor.apply()

                        val geocodingResponseJsonString2=msharedPreferences.getString(Constants.GEO_CODING_RESPONSE_DATA,"")
                        val geoList:List<GeocodingResponseItem> = Gson().fromJson(geocodingResponseJsonString,Array<GeocodingResponseItem>::class.java).toList()

                        val latitude:Double=geoList[0].lat
                        val longtitude:Double=geoList[0].lon
